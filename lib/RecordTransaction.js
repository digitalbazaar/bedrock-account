...
export class RecordTransaction {
  constuctor({type, id, record, data, meta, helper} = {}) {
    ...
    this.prepareCommit = this[`_prepare_${type}`];
  }
  ...
  async run() {
    try {
      ...
      const update = await this.prepareCommit();
  ...
  async _prepare_insert() {
    // create `update` parameters
    const {id, txn} = this;
    const newTxn = {...txn, committed: true};
    // FIXME: update must remove `_pending` as well
    const update = {id, oldTxn: txn, newTxn};
    return update;
  }

  async _prepare_update() {
    // create `update` parameters
