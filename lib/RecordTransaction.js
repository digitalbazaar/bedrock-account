...
export class RecordTransaction {
  constructor({type, id, record, data, meta, recordCollection} = {}) {
  ...
  async run() {
    ...
    try {
      ...
      await tp.updateProxyRecords({record});

      // commit transaction (write to data record w/actual changes)
      await tp.commitTransaction({id, record, data, meta, txn});
