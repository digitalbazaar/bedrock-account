...
const validateCapabilityDelegation =
  promisify(brIdentity.validateCapabilityDelegation);
 ...
 */
api.insert = brCallbackify(async ({actor, account, meta}) => {
  ...
  for(let i = 0; i < roles.length; ++i) {
    ...
    if(role.generateResource === 'id') {
      ...
      throw new
      BedrockError('Could not create Account; unknown ResourceRole rule.',
  ...
  if(meta.sysResourceRole.length > 0) {
    const caps = {actor, resourceRoles: meta.sysResourceRole};
    await validateCapabilityDelegation(caps);
  }
  ...
  try {
    const result = await database.collections.account
 ...
 */
api.exists = brCallbackify(async ({actor, id, email, status = 'active'}) => {
...
api.get = brCallbackify(async ({actor, id}) => {
  ...
  const record = await database.collections.account
    .findOne({id: database.hash(id)}, {_id: 0, account: 1, meta: 1});
  if(!record) {
    throw new BedrockError('Account not found.',
      'NotFoundError',
...
api.getAll = brCallbackify(async (
  ...
  const records = await database.collections.account
    .find(query, fields, options).toArray();
...
api.update = brCallbackify(async ({actor, id, patch, sequence}) => {
  ...
  if(record.meta.sequence !== sequence) {
    throw new
    BedrockError('Could not update Account. Record sequence does not match.',
      'InvalidStateError', {
        httpStatusCode: 409,
        public: true,
        actual: sequence,
  ...
  const customValidate = (operation, index, tree, existingPath) => {
    ...
    if(pathId) {
      throw new jsonpatch.JsonPatchError('"id" cannot be changed',
  ...
  if(errors) {
    throw new
    BedrockError('The given JSON patch is invalid.',
      'ValidationError', {
        httpStatusCode: 400,
        public: true,
        patch,
  ...
  if(result.result.n === 0) {
    // this is what func-paren never set. we might want to define these errors
    // in their own file so we can reuse.
    return new
    BedrockError('Could not update Account. Record sequence does not match.',
...
api.setStatus = brCallbackify(async ({actor, id, status}) => {
  ...
  if(result.result.n === 0) {
    throw new
    BedrockError('Could not set Account status. Account not found.',
      'NotFoundError',
...
api.updateRoles = brCallbackify(async (
  ...
  const {account, meta} = await api.get({actor: null, id});
  ...
  for(const role of changes) {
    if(role.generateResource === 'id') {
      ...
      throw new
      BedrockError('Could not set roles; unknown ResourceRole rule.',
        'NotSupportedError', {sysResourceRole: role});
    }
  }
  ...
  // TODO this causes alot of trouble with no func-paren new lines allowed
  const resourceRoles = _.uniqWith(brPermission.
    mergeCapabilities(brPermission.
      subtractCapabilities(meta.sysResourceRole, remove), add),
  _.isEqual);

  await database.collections.account
    .update({id: database.hash(id)}, {
...
api.getCapabilities = brCallbackify(async ({id, identities}) => {
  ...
  const record = await database.collections.account
    .findOne({id: database.hash(id)}, {_id: 0, 'meta.sysResourceRole': 1});
  ...
  const records = await database.collections.identity
    .find(query, {_id: 0, 'meta.sysResourceRole': 1}).toArray();
  ...
  records
    .map(r => r.meta.sysResourceRole)
    .forEach(capabilities => actor.sysResourceRole.push(...capabilities));
...
api.manageIdentity = brCallbackify(async ({actor, accountId, identityId}) => {
  ...
  const {identity} = await brIdentity.get({actor, id: identityId});
  await _checkPermission({
    actor,
  ...
  const result = await database.collections.identity
    .update({id: database.hash(identityId)},
  ...
  if(result.result.n === 0) {
    throw new
    BedrockError('Could not set Account as manager of Identity. Identity not found.',
      'NotFoundError', {
        httpStatusCode: 404,
        identity: identityId,
        account: accountId,
 ...
 *
 * @param actor the actor or capabilities for performing the action.
 * @param identity the ID of the identity to get the managing Account for.
 *
 * @return a Promise that resolves to the account ID or `null`.
 */
api.getManagerId = brCallbackify(async ({actor, identity}) => {
  assert.string(identity, 'identity');

  const record = await database.collections.identity.findOne({
    id: database.hash(identity),
    'meta.bedrock-account.account': {$exists: 1}
  }, {
    _id: 0,
    'identity.id': 1,
    'identity.owner': 1,
    'meta.bedrock-account.account': 1
  });

  await _checkPermission({
    actor,
    permission: PERMISSIONS.IDENTITY_ACCESS,
    resource: [record ? record.identity : identity],
    translate: 'owner'
  });

  return record ? record.meta['bedrock-account'].account : null;
});
