 ...
 */
exports.exists = async ({actor, id, email, status = 'active'} = {}) => {
...
exports.get = async ({actor, id} = {}) => {
  ...
  const record = await database.collections.account.findOne(
    {id: database.hash(id)}, {projection: {_id: 0, account: 1, meta: 1}});
  if(!record) {
    throw new BedrockError(
      'Account not found.',
      'NotFoundError',
 ...
 */
exports.getAll = async ({actor, query = {}, fields, options = {}} = {}) => {
...
exports.getCapabilities = async ({id} = {}) => {
  ...
  const record = await database.collections.account.findOne(
    {id: database.hash(id)}, {projection: {_id: 0, 'meta.sysResourceRole': 1}});
