 ...
 */
exports.insert = brCallbackify(async ({actor, account, meta}) => {
 ...
 */
exports.exists = brCallbackify(async (
 ...
 */
exports.get = brCallbackify(async ({actor, id}) => {
 ...
 */
exports.getAll = brCallbackify(async (
  ...
  if(fields && options.projection) {
    throw new TypeError('"fields" or "options.projection" must be given, not both.');
  }
 ...
 */
exports.update = brCallbackify(async ({actor, id, patch, sequence}) => {
  assert.string(id, 'id');
 ...
 * @returns {Promise} Resolves once the operation completes.
 */
exports.setStatus = brCallbackify(async ({actor, id, status}) => {
  assert.string(id, 'id');
  assert.string(status, 'status');
 ...
 * @returns {Promise} Resolves once the operation completes.
 */
exports.updateRoles = brCallbackify(async (
 ...
 *   for.
 * @param {Array} [options.identities] - A set of identity IDs to restrict
 *   capabitilies to; if unspecified, all identity capabilities will be
 *   included.
 *
 * @returns {Promise} Resolves to an `actor` once the operation completes.
 */
exports.getCapabilities = brCallbackify(async ({id, identities}) => {
  assert.string(id, 'id');
  ...
  const records = await database.collections.identity.find(
    query, {_id: 0, 'meta.sysResourceRole': 1}).toArray();
 ...
 * @param {Actor} options.actor - The actor or capabilities to perform the
 *   action.
 * @param {string} options.accountId - The ID of the account to make the
 *   manager of an identity.
 * @param {string} options.identityId - The ID of the identity to manage.
 *
 * @returns {Promise} Resolves once the operation completes.
 */
exports.manageIdentity = brCallbackify(async (
  {actor, accountId, identityId}) => {
 ...
 * @param {Actor} options.actor - The actor or capabilities for performing
 *   the action.
 * @param {string} options.identity - The ID of the identity to get the
 *   managing Account for.
 *
 * @returns {Promise} Resolves to the account ID or `null`.
 */
exports.getManagerId = brCallbackify(async ({actor, identity}) => {
