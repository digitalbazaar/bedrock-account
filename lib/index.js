...
const {brCallbackify} = bedrock.util;
...
bedrock.events.on('bedrock-mongodb.ready', callback => callbackify(async () => {
...
api.exists = brCallbackify(async ({actor, id, deleted = false}) => {
  await _checkPermission(actor, PERMISSIONS.ACCOUNT_ACCESS, {resource: [id]});
  const query = {
    id: database.hash(id),
    'meta.sysStatus': deleted ? 'deleted' : 'active'
  };
  return !!await database.collections.account.findOne(query, {id: 1});
});
...
api.insert = brCallbackify(async ({actor, account}) => {
  ...
  // ensure action is permitted
  await _checkPermission(
    actor, PERMISSIONS.ACCOUNT_INSERT, {resource: [account]});
...
api.getAll = brCallbackify(async (
  ...
  await _checkPermission(actor, PERMISSIONS.ACCOUNT_ACCESS);
...
api.update = brCallbackify(async ({actor, id, patch, sequence}) => {
  ...
  await _checkPermission(actor, PERMISSIONS.ACCOUNT_UPDATE, {resource: [id]});
  ...
  await promisify(validateInstance)(patched, accountSchema, patched);

  // TODO: can't permit any account to claim identities ... maybe `identity`
  // should not be present on `account` record but only on `identity` record
  // in its meta? ... identityRecord.meta.account ?
...
api.setStatus = brCallbackify(async ({actor, id, status}) => {
  await _checkPermission(
    actor, PERMISSIONS.ACCOUNT_META_UPDATE, {resource: [id]});

  const result = await database.collections.account.update(
    {id: database.hash(id)},
    {$set: {'meta.sysStatus': status}},
    database.writeOptions);
...
api.manageIdentity = brCallbackify(async ({actor, accountId, identityId}) => {
  await _checkPermission(
    actor, PERMISSIONS.IDENTITY_EDIT, {resource: [identityId]});
 ...
 */
async function _checkPermission({actor, permission, options}) {
  ...
  const record = await database.collections.account.findOne(
    {id: database.hash(actor.id)}, {'meta.sysResourceRole': true});
