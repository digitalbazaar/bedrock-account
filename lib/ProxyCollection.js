...
export class ProxyCollection {
  ...
  async insert({uniqueValue, recordId, txnId} = {}) {
    ...
    if(txnId) {
      record._txn = {id: txnId, op: 'insert'};
    }
  ...
  async get({recordId, uniqueValue, explain = false} = {}) {
    ...
    const idField = this._getDataIdField();
    const collection = this._getCollection();
    // FIXME: determine if `_txn.op !== insert` needs to be checked unless
    // specified via an internal flag here
    const query = {};
  ...
  async rollback({txnId} = {}) {
    assert.string(txnId, 'txnId');
    await Promise.all([
      this._rollbackInserts({txnId}),
      this._rollbackRemoves({txnId})
    ]);
  }
  ...
  async _rollbackInserts({txnId} = {}) {
    const collection = this._getCollection();
    const query = {'_txn.id': txnId, '_txn.op': 'insert'};
    await collection.deleteMany(query);
  }

  async _rollbackRemoves({txnId} = {}) {
    const collection = this._getCollection();
    const query = {'_txn.id': txnId, '_txn.op': 'remove'};
