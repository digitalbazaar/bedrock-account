...
export class ProxyCollection {
  ...
  async insert({uniqueValue, recordId, txnId} = {}) {
    ...
    const idField = this._getDataIdField();
    // FIXME: consider adding `_pending` here to make rollback updates simpler
  ...
  async rollbackRemove({recordId, txnId, explain = false} = {}) {
    // FIXME: consider changing function name to `rollback` and auto-delete
    // any mapping records with `_pending` and update any records without it
    // to remove `txnId`
    assert.string(recordId, 'recordId');
    assert.string(txnId, 'txnId');
