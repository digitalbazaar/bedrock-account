...
export class RecordCollection {
  ...
  async get({
    ...
    while(true) {
      ...
      // note that a pending record is not retrieved during an `update` or
      // a regular `get`; it will only be rolled back (if necessary) when
      ...
      if(record._txn) {
        ...
        await tp.processPendingTransaction({record, _awaitRollback: false});
