...
export class RecordCollection {
  ...
  async _completeTransaction({record} = {}) {
    ...
    // FIXME: tweak algorithm; may instead want to find all mapping records
    // that match `record.[dataField].id` to see if any have a `txnId`...
    // FIXME: or, alternatively, only run `_completeTransaction` when a `txnId`
    // is found in a mapping as the result of trying to do an update -- so we
    // optimize for the case where there are no incomplete transactions

    1. Read data record and unique mapping records with `uniqueValue` and
      `txnId`.
    2. If no records are found, finished, return.
    3. If data record not found, it is safe to assume that the unique mapping
      records are related to a record deletion. This is because only updates
      and deletions use a `txnId`, inserts do not. Remove unique mapping records
      gated on `txnId` and return regardless of return value (but throw on
      exception).
    4. If data record is found, it is safe to assume the transaction is an
      update because an insert would have no matching `txnId` and a deletion
      would have removed the data record, not set a `txnId` on it. Concurrently
      remove the unique mapping records that no longer match the data record
      and update any other unique mapping records, both gated on `txnId`.
    5. Update the data record to remove `txnId`, gated on `txnId`.
    6. Finished, return. */
    const {dataField, proxyCollections} = this;
    const {[dataField]: data} = record;
