...
export class RecordCollection {
      ...
      if(data && proxyCollections.size > 0) {
        ...
        try {
          ...
          const {txnId, result} = await this._tryTransaction(
            {id, data, meta, record, sequenceLocation, expectedSequence});
          if(txnId) {
  ...
  async _tryTransaction({id, data, meta, record} = {}) {
    ...
    */
    ...
    if(!rollback) {
      ...
      if(!await this._update({id, data, meta, newTxn})) {
  ...
  async _update({
    id, data, meta, newTxn, oldTxn, sequenceLocation, expectedSequence,
    explain = false
  } = {}) {
    const {dataField, sequenceInData} = this;
