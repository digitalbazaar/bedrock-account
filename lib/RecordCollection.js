...
export class RecordCollection {
  ...
  async _rollbackTransaction({id, txnId} = {}) {
    ...
    // FIXME: roll back all proxy collection updates associated with `txnId`
  }
  ...
  async _tryTransaction({
    ...
    */
    ...
    if(rollback) {
      // FIXME: this can't be called if the data record has a `txn` already;
      // need to adjust the logic
      await this._rollbackTransaction({record, data});

      // complete (finish or abort as appropriate) any pending transactions;
      // FIXME: note that can't be done in parallel because they'd stomp on
      // each other... call `_completeTransaction({record})` on every
      // `pendingTxn`
      if(pendingTxns.length === 0) {
        // FIXME: note when this is called without params it uses the `txnId`
        // from the data record
        await this._completeTransaction();
      } else {
        // get txn IDs in parallel, but complete each transaction in series
        //const txnIds = [];
        // ...
        // FIXME: complete each txn
      }
