...
export class RecordCollection {
  ...
  async insert({record} = {}) {
    ...
    // FIXME: perhaps disable this for now and add it back as an optimization
    // in the future -- it needs to ensure any pending transaction is
    // processed if a duplicate error occurs
  ...
  async get({id, uniqueField, uniqueValue, _allowPending = false} = {}) {
    ...
    console.log('required proxy collection', proxyCollection);
    while(true) {
      if(proxyCollection) {
