...
export class RecordCollection {
  ...
  async insert({record} = {}) {
    ...
    console.log('applyUniqueConstraint', applyUniqueConstraint);
  ...
  async get({id, uniqueField, uniqueValue, _allowPending = false} = {}) {
    ...
    // FIXME: set limit on attempts
    const {helper, transactionProcessor: tp} = this;
    while(true) {
      if(proxyCollection) {
        ...
        if(proxyRecord._txn) {
          ...
          await tp.processAnyPendingTransaction(
  ...
  async update({id, data, meta} = {}) {
    ...
    this.helper.validateUpdateParams({id, data, meta});
    ...
    while(true) {
      try {
        ...
        const t = new RecordTransaction(
          {id, record, data, meta, type: 'update', recordCollection: this});
        return await t.run();
      } catch(e) {
        if(e.name !== 'AbortError') {
          // unrecoverable error
   ...
   *
   * @returns {Promise<boolean>} Resolves with `true` on delete success.
   */
  async delete({id} = {}) {
    assert.string(id, 'id');
