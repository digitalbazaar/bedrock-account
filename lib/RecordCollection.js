...
export class RecordCollection {
  ...
  async update({id, data, meta, explain} = {}) {
    ...
    "transaction" must be performed to properly update the record and any
    proxy collections.

    // FIXME: determine best location for these docs
  ...
  async _startTransaction({data, record} = {}) {
    ...
    */
    ...
    if(!rollback) {
      // FIXME: PROBLEM: this needs to execute the `update` call that runs
      // after `_startTransaction` in `update()`... so perhaps can't run this
      // in a helper or just needs refactoring to do so
      // FIXME: write `txnId, rollback: false` to data record
      // FIXME: if update failed, set rollback to true
    }
    ...
    if(rollback) {
      ...
      // FIXME: return (but ensure loop occurs ... so need to return a value)
    } else {
